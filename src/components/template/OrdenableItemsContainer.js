    ...
    >
      <div className="w-full  relative ">
        <div className="w-full h-full relative flex flex-col items-center justify-center ">
          {/*    {configData.nftCollection.orderType !== undefined && (
            <SortItemsFilters
              itemsList={itemList}
              ...
              sortOptions={configData.nftCollection.orderType?.sort}
              viewOptions={configData.nftCollection.orderType?.view}
            />
          )}
 */}
          {viewSelected === "GRID" ? (
      ...
      <div className="w-100 mt-10">
        {scrolled && (
          <button
            onClick={() => goToTop()}
            className="absolute bottom-5 right-10 border vborder-white p-2 rounded-full bg-bg-3 cursor-pointer hover:animate-bounce"
          >
